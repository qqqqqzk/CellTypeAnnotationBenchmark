...
rule transferdata:
  ...
  shell:
    "Rscript Scripts/run_transferdata.R "
    "{input.datafile} "
    "{input.labfile} "
    "{input.folds} "
...
rule scPred:
  input:
    ...
    ranking = feature_ranking
  output:
    pred = "{output_dir}/scPred/scPred_Pred_Labels.csv",
...
#    labfile = config["labfile"],
#    folds = "{output_dir}/CV_folds.RData",
#    #genes_names = config.get("genes", "gene_UNSPECIFIEDFILE"),
#    #markers = config.get("Garnett_CV", {}).get(
#    #    "markers", "marker_UNSPECIFIEDFILE")
